...
if opts[:pruneolderthan]
  beforePrune = Dir.glob("#{opts[:backupdir]}/#{opts[:backupname]}*").count
  log.info("Removing backups older than #{opts[:pruneolderthan]} days:")  
  Dir.glob("#{opts[:backupdir]}/#{opts[:backupname]}*").each {|f|
  ...
  afterPrune = Dir.glob("#{opts[:backupdir]}/#{opts[:backupname]}*").count
